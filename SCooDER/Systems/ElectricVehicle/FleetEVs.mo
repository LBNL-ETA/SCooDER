...
model FleetEVs
  parameter Integer NumberEVs = 1 "Amount of EVs on site";

  Modelica.Blocks.Interfaces.RealInput PPlugCtrl[NumberEVs] "Control of individual EVs when plugged in [W]"
    annotation (Placement(transformation(extent={{-140,34},{-100,74}})));
  Modelica.Blocks.Interfaces.RealInput PluggedIn[NumberEVs] "This input sets an individual EV as plugged in, when >= 1"
    annotation (Placement(transformation(extent={{-140,-2},{-100,38}})));
  Modelica.Blocks.Interfaces.RealInput PDriveCtrl[NumberEVs] "Control of individual EVs when driving [W]"
  ...
  Components.ElectricVehicle.EV eV[NumberEVs]
